...
dataset_flow._init_from_file('data_pairs.py')
...
model.compile(optimizer=tf.keras.optimizers.Adam(lr=1e-5), loss='categorical_crossentropy')
model_checkpoint = tf.keras.callbacks.ModelCheckpoint('model_checkpoint/lstm_model3.h5', monitor='loss', mode='min', verbose=1, save_best_only=True, save_weights_only=False)
reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(monitor='loss', factor=0.1, patience=10, min_lr=1e-5, verbose=1, mode='min')
model.fit(dataset_flow.input_tensor, dataset_flow.target_tensor, epochs=100, batch_size=2, callbacks=[model_checkpoint, reduce_lr])